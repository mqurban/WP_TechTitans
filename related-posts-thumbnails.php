...
function get_first_post_image() {
    ...
    // If there are attachments, return the first image URL
    if ( $attachments ) {
        foreach ( $attachments as $attachment ) {
            return wp_get_attachment_image_src( $attachment->ID, 'full' )[0];
        }
    }

    // If no attachments found, return null
    return null;
}

// Function to set post thumbnail
function set_post_thumbnail_from_first_image_or_default() {
    global $post;

    // Get the first post image
    $first_image_url = get_first_post_image();

    // If first image exists, set it as post thumbnail
    if ( $first_image_url ) {
        set_post_thumbnail( $post->ID, $first_image_url );
    } else {
        // If no first image, set default image as post thumbnail
        $default_image = ''; 
        set_post_thumbnail( $post->ID, $default_image );
    }
	return $first_image_url;
}
// Hook into the save_post action to set the post thumbnail
add_action( 'save_post', 'set_post_thumbnail_from_first_image_or_default' );





function previous_posts_meta_box() {
    add_meta_box(
        'previous-posts-box',
...
add_action('add_meta_boxes', 'previous_posts_meta_box');

// Display the content of the meta box
function display_previous_posts($post) {
    ...
    $selected_posts = get_post_meta($post->ID, '_selected_posts', true);

    // Ensure $selected_posts is an array, default to an empty array if not
    $selected_posts = is_array($selected_posts) ? $selected_posts : array();

    if ($previous_posts) {
        echo '<ul>';
        foreach ($previous_posts as $previous_post) {
            $checked = in_array($previous_post->ID, $selected_posts) ? 'checked' : '';

            echo '<li ><input type="checkbox" name="selected_posts[]" value="' . esc_attr($previous_post->ID) . '" ' . $checked . '> ' .
                '<a style="text-decoration:none;" href="' . get_permalink($previous_post->ID) . '">' . esc_html($previous_post->post_title) . '</a></li>';
        }
        echo '</ul>';
    } else {
...
// Save the selected posts when the post is saved or updated
...
// Count function
function get_selected_posts_count() {
    global $post;
    $selected_posts = get_post_meta($post->ID, '_selected_posts', true);
    if ($selected_posts) {
        return count($selected_posts);
    }
    // If no selected posts, return 0
    return 0;
}

// Display function
function display_selected_posts($content) {
	if(is_single()){
	global $post;
	$relpoststh_fontsize = get_option( 'relpoststh_fontsize' );
	$relpoststh_fontfamily = get_option( 'relpoststh_fontfamily' );
	$relpoststh_textblockheight = get_option( 'relpoststh_textblockheight' );
	$relpoststh_show_date = get_option( 'relpoststh_show_date' );

    $selected_posts = get_post_meta($post->ID, '_selected_posts', true);

    if ($selected_posts) {
        $selected_posts_content = '<div>' . stripslashes( htmlspecialchars( get_option( 'relpoststh_top_text') ) ) . '</div>';

		$selected_posts_content .= '<div class="relpost-custom-container">';
        foreach ($selected_posts as $selected_post_id) {
			if ( get_option( 'relpoststh_disable_images' ) ) {
				// Display related posts as a bullet list without images
				$selected_posts_content .= '<ul class="related-posts-list">';
				$selected_posts_content .= '<li><a href="' . get_permalink( $selected_post_id ) . '">' . esc_html( get_the_title( $selected_post_id ) ) . '</a></li>';
				$selected_posts_content .= '</ul>';
			}
			else {
			$selected_posts_content .= '<div style="width: 23%; padding: 5px; border: 1px solid #ddd; border-radius: 5px;  float: left; transition: background-color 0.3s;" onmouseover="this.style.backgroundColor=\'#EEEEEE\'" onmouseout="this.style.backgroundColor=\'\'">';
            $selected_posts_content .= '<a href="' . get_permalink($selected_post_id) . '" style="text-decoration: none; color: #333; display: block;">';
			$selected_posts_content .= '<div style="width: 100%; height: 0; padding-bottom: 100%; position: relative; overflow: hidden; margin-bottom: 10px;">';
			if (has_post_thumbnail($selected_post_id)) {
				$thumbnail_url = get_the_post_thumbnail_url($selected_post_id, 'thumbnail');
			} 
			
			else{
				// Use the default image URL
				$thumbnail_url = get_option( 'relpoststh_default_image' );
			}
			$selected_posts_content .= '<div style="background-image: url(' . esc_url($thumbnail_url) . '); background-size: cover; width: 100%; height: 100%; position: absolute; top: 0; left: 0;"></div>';
			$selected_posts_content .= '</div>';
            $categories = get_the_category($selected_post_id);
            $post_date = get_the_date('F j, Y', $selected_post_id);
            $selected_posts_content .= '<div style="font-size: ' . $relpoststh_fontsize . 'px; font-family:'. $relpoststh_fontfamily .'; line-height: 0.6; text-align:center; height:'. $relpoststh_textblockheight .'px;">';
			$selected_posts_content .= '<p style="margin-bottom: 5px;  ">' . esc_html((get_the_title($selected_post_id))) . '</p>';
			if ($relpoststh_show_date) {
				$selected_posts_content .= '<p> ' . esc_html($post_date) . '</p>';
			}
			else {
			...
			{
            if ($categories) {
                $selected_posts_content .= '<p style="margin-bottom: 5px;">';
                foreach ($categories as $category) {
                    $selected_posts_content .= '<p ' . esc_url(get_category_link($category->term_id)) . '>' . esc_html($category->name) . '</p> ';
                }
                $selected_posts_content .= '</p>';
            } }
			else {

			}
            $selected_posts_content .= '</div>';
            $selected_posts_content .= '</a></div>';
		}}
        $selected_posts_content .= '</div>';
        $content .= $selected_posts_content;
    }

    return $content;}
}
add_filter('the_content', 'display_selected_posts');
...
class RelatedPostsThumbnails {
	...
	*/
	...
	function get_thumbnails( $show_top = false, $posts_number = '', $sort_by = '', $main_title = '', $exclude = '',$selected_posts = '' ) {
		$output       = '';
		...
		*/
		...
		if ( $show_top ) {
			...
			$selected_posts_count = display_selected_posts('');
...
add_action('wp_footer', 'display_author_related_posts_script');

function display_author_related_posts_script() {
    // Check if we are on an author archive page
    if (is_author()) {
        ?>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                heading.innerHTML = 'Author Related Posts';
                heading.style.fontSize = '20px';
                container.appendChild(heading);

                // Find the footer dynamically
                var footer = document.querySelector('footer');

                if (footer) {
                    // Insert the container above the footer
